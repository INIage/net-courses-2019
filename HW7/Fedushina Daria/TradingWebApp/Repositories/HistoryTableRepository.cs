...
namespace TradingConsoleApp.Repositories
{
    class HistoryTableRepository : IHistoryTableRepository
    {
        ...
        public List<TransactionHistoryEntity> GetAllFromBase()
        {
            List<TransactionHistoryEntity> arr = new List<TransactionHistoryEntity>();
            foreach (TransactionHistoryEntity entity in this.dbContext.Transactions)
            {
                arr.Add(entity);
            }
            return arr;
        ...
        public int GetId(TransactionInfo transactionInfo)
        {

            var entity = this.dbContext.Transactions.First(f =>
           f.BuyerBalanceID == transactionInfo.BuyerBalanceID &&
           f.SellerBalanceID == transactionInfo.SellerBalanceID);
            return entity.TransactionID;
