namespace TradingApp.Core.Tests
{
    ...
    public class UserServiceTest
    {
        ...
        public void ShouldUpdateUser()
        {
            ...
            userTableRepository.GetUserById(1).Returns(baseUser);
            UserInfo args = new UserInfo()
            {
                Name = "test"
            };

            usersService.Update(1, args);

            userTableRepository.Received(1).GetUserById(1);
            userTableRepository.Received(1).SaveChanges();
            Assert.AreEqual(baseUser.Name, args.Name);
        }
        ...
        public void ShouldRemoveUser()
        {
            ...
            UsersService usersService = new UsersService(userTableRepository);
            UserEntity user = null;
            userTableRepository.GetUserById(1).Returns(user);

            usersService.Remove(1);

            userTableRepository.Received(1).Remove(1);
            userTableRepository.Received(1).SaveChanges();
        }
