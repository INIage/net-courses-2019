namespace TradingAppWebAPI.Repos
{
    ...
    class ClientRepository : DBComm, IClientRepository
    {
        ...
        {
            dBContext.Clients.Remove(GetClientByID(clientID));
            dBContext.SaveChanges();
        }

        public void Update(Client newClient)
        {
            var oldClient = GetClientByID(newClient.ClientID);
            dBContext.Entry(oldClient).CurrentValues.SetValues(newClient);
            dBContext.SaveChanges();
        }
    }
}